...
// Keep track of mute state and saved volume
static bool is_muted = false;
static gdouble saved_volume = 1.0; // default full volume

void on_mute_clicked(GtkWidget *button, gpointer user_data) {
    PlayAudio* player = PlayAudio::getInstance();
    GstElement* volume_elem = player->getVolumeElement(); // use the volume element
    if (!volume_elem) {
