   ...
   goom(nullptr), volume_element(nullptr), bus(nullptr), msg(nullptr)
{
    ...
    GstPad *tee_vis_pad = gst_element_request_pad_simple(tee, "src_%u");
...
void PlayAudio::play_audioFile(string_view file_path, string_view folder_path, GtkWidget* window)
{
    ...
    if (!window) {
        g_printerr("ERROR: GTK window is NULL\n");
        return;
    }
    ...
    if (stat(file_path_str.c_str(), &file_stat) == -1) {
        g_printerr("ERROR: File not found or not accessible: %.*s\n", static_cast<int>(file_path.size()), file_path.data());
        ...
        gtk_message_dialog_format_secondary_text(
            GTK_MESSAGE_DIALOG(error_dialog),
            "Cannot access file:\n%.*s", static_cast<int>(file_path.size()), file_path.data());
    ...
    if (instance.pipeline) {
        ...
        g_print("Previous song stopped\n");
    }

    // Set the file path in the existing filesrc
    if (!instance.filesrc) {
        g_printerr("ERROR: filesrc element is NULL\n");
        return;
    }

    g_object_set(instance.filesrc, "location", file_path_str.c_str(), NULL);
...
if (favButton) {
    ...
    const char* icon_name = "non-starred-symbolic";
    if (favourites && favourites->isFavourite(filename)) {
...
bool PlayAudio::isPlaying() const {
...
// {
//     if (player_state::current_song_index + 1 < player_state::song_list.size()) {
...
//     }
//      string filepath=player_state::folder_path+"/" +song_list[player_state::current_song_index];
//      //   cout<<filepath<<endl;
//     //    play_audioFile(filepath,player_state::folder_path,window);
// }
...
void PlayAudio::handleEOS(GtkWidget* window) {
    ...
    if (player_state::repeat_mode) {
        ...
        next_file = player_state::folder_path + "/" + player_state::song_list[player_state::current_song_index];
        ...
        if (player_state::song_list.size() > 1) {
            int rand_index;
            do {
                rand_index = rand() % player_state::song_list.size();
            } while (rand_index == player_state::current_song_index);

            player_state::current_song_index = rand_index;
        }
        ...
        player_state::current_song_index++;
        if (static_cast<size_t>(player_state::current_song_index) >= player_state::song_list.size()) {
            player_state::current_song_index = 0; // loop back to beginning
        }
        next_file = player_state::folder_path + "/" + player_state::song_list[player_state::current_song_index];
    }

    // Play the next song
    play_audioFile(next_file, player_state::folder_path, window);
}
